...
const NAV = [
  ...
  },
  {
    ...
    children: [
      { label: 'Productos',          href: '/dashboard/inventario', icon: '📦', roles: ['admin', 'bodeguero'] },
      { label: 'Facturas de compra', href: '/dashboard/compras',    icon: '🧾', roles: ['admin', 'bodeguero'] },
      { label: 'Proveedores',        href: '/dashboard/proveedores',icon: '🚚', roles: ['admin'] },
    ],
  },
  {
    type: 'group',
    label: 'Distribución', icon: '🚛',
      ...
      <nav style={{ flex: 1, padding: '8px', overflowY: 'auto' }}>
        {filteredNav.map(item => {
          ...
          if (item.type === 'group') {
            ...
            const hasActive    = filteredChildren.some(c => c.href === pathname)
      ...
      <div style={{ padding: '8px', borderTop: '1px solid var(--border)' }}>
        ...
        {collapsed && (
          <button
            ...
            title="Cambiar tema"
            style={{ width: '100%', padding: '9px', background: 'transparent', border: '1px solid var(--border)', borderRadius: 8, cursor: 'pointer', fontSize: 16, marginBottom: 8 }}
          >
            🌙
          </button>
        )}
        <button
          ...
          style={{
            ...
            display: 'flex', alignItems: 'center',
            justifyContent: collapsed ? 'center' : 'flex-start',
            gap: collapsed ? 0 : 8, transition: 'border-color .15s, color .15s',
          }}
